...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-zinc-200 font-sans pb-24 flex flex-col items-center">
      {/* HEADER */}
      <header className="sticky top-0 z-40 w-full max-w-md bg-[#121212]/95 backdrop-blur-md border-b border-white/5 px-4 py-4 flex justify-between items-center">
        ...
        {activeTab === 'home' && (
          <div className="space-y-6 animate-in fade-in">
            <SimpleCard className="p-8 flex flex-col items-center">
              <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-3">今月の残り (カード)</p>
              <h2 className={`text-5xl font-bold mb-6 ${summary.cardRemaining < 0 ? 'text-red-400' : 'text-white'}`}>¥{summary.cardRemaining.toLocaleString()}</h2>
              <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-white transition-all duration-1000" style={{ width: `${summary.cardPercent}%` }} />
              </div>
            </SimpleCard>

            <SimpleCard className="p-8 flex flex-col items-center">
              <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-3">今月の残り (現金予算)</p>
              <h2 className={`text-4xl font-bold mb-6 ${summary.cashRemaining < 0 ? 'text-red-400' : 'text-white'}`}>¥{summary.cashRemaining.toLocaleString()}</h2>
              <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-zinc-400 transition-all duration-1000" style={{ width: `${summary.cashPercent}%` }} />
              </div>
            </SimpleCard>
              ...
              <div key={t.id} className="flex justify-between items-center bg-[#1E1E1E] p-4 rounded-lg border border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="max-w-[120px] overflow-hidden">
                    ...
                    <div className="text-[10px] text-zinc-500 font-bold uppercase">{t.category}</div>
                  </div>
                </div>
            ...
            {settingTab === 'menu' && (
              <div className="space-y-2">
                {[
                  ...
                  { id: 'payment', label: 'カード・支払方法編集', icon: <Wallet size={18}/> },
                ].map(item => (
            ...
            {settingTab === 'budget' && (
              <div className="space-y-4">
                <SimpleCard className="p-8 space-y-6">
                  <h3 className="text-xs font-bold text-zinc-400 text-center uppercase tracking-widest">全体の軍資金</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] text-zinc-500 mb-1 block uppercase">今月の軍資金 (カード用)</label>
                    ...
                    <div>
                      <label className="text-[10px] text-zinc-500 mb-1 block uppercase">今月の軍資金 (現金用)</label>
                      <input type="number" defaultValue={monthlyData.cashBudget} onBlur={e => setDoc(doc(db,'users',SHARED_USER_ID,'months',month),{cashBudget:Number(e.target.value)},{merge:true})} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg p-3 text-center text-white outline-none" />
                    </div>
                  </div>
                </SimpleCard>

                <SimpleCard className="p-8 space-y-6">
                  <h3 className="text-xs font-bold text-zinc-400 text-center uppercase tracking-widest">カード別 請求額</h3>
                  <div className="space-y-4">
                    {config.paymentMethods.filter(m => m !== '現金').map(card => (
                      <div key={card}>
                        <label className="text-[10px] text-zinc-500 mb-1 block uppercase tracking-tight">{card} の請求額</label>
                        <input 
                          type="number" 
                          ...
                          className="w-full h-11 bg-black/20 border border-white/10 rounded-lg p-3 text-center text-white outline-none font-mono" 
                        />
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  {(monthlyData.fixedCosts || []).map(f => (
                    <div key={f.id} className="flex justify-between items-center bg-white/5 p-4 rounded-lg border border-white/5">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="font-mono text-sm text-white whitespace-nowrap">¥{f.amount.toLocaleString()}</span>
                  ...
                  <div className="flex gap-2 pt-6 h-11">
                    <input id="fx-n" placeholder="費目" className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none min-w-0" />
                    <input id="fx-a" type="number" placeholder="金額" className="w-24 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none text-center min-w-0" />
                    <button onClick={() => {
                      ...
                      n.value=''; a.value='';
                    }} className="bg-zinc-200 text-black px-3 rounded-lg font-bold text-[10px] uppercase whitespace-nowrap">追加</button>
                  </div>
                </div>
              </SimpleCard>
            )}
              ...
              <SimpleCard className="p-8 space-y-6 text-center">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 h-11">
                    <input id="new-cat" placeholder="新カテゴリ" className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none" />
                    <button onClick={() => {
                      const i=document.getElementById('new-cat');
                      if(i.value) setDoc(doc(db,'users',SHARED_USER_ID,'settings','config'),{...config,categories:[...config.categories,i.value]});
                      i.value='';
                    }} className="bg-zinc-200 text-black px-4 rounded-lg font-bold text-[10px] uppercase">追加</button>
                  </div>
                </div>
              </SimpleCard>
            )}
            ...
            {settingTab === 'payment' && (
              <SimpleCard className="p-8 space-y-6 text-center">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 h-11">
                    <input id="new-pay" placeholder="カード名等" className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none" />
                    <button onClick={() => {
                      const i=document.getElementById('new-pay');
                      if(i.value) setDoc(doc(db,'users',SHARED_USER_ID,'settings','config'),{...config,paymentMethods:[...config.paymentMethods,i.value]});
                      i.value='';
                    }} className="bg-zinc-200 text-black px-4 rounded-lg font-bold text-[10px] uppercase">追加</button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <SimpleCard className="relative w-full max-w-md p-6 space-y-6">
            ...
            <form onSubmit={handleTxSubmit} className="space-y-5">
              ...
              <input name="title" type="text" defaultValue={editingTx?.title || ''} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg p-3 text-sm text-center text-white outline-none" placeholder="タイトル (例: ランチ)" />
              <div className="grid grid-cols-2 gap-4 h-11">
                <input name="date" type="date" defaultValue={editingTx ? editingTx.date.split('T')[0] : getTodayString()} className="bg-black/20 border border-white/10 rounded-lg text-xs text-center text-white outline-none p-1" />
                <select name="category" defaultValue={editingTx?.category || config.categories[0]} className="bg-black/20 border border-white/10 rounded-lg text-xs text-center text-white outline-none p-1 appearance-none">
                  {config.categories.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="flex flex-wrap gap-2 justify-center">
                {config.paymentMethods.map(m => (
                  <label key={m} className="cursor-pointer">
                    ...
                    <div className="px-4 h-10 text-center rounded-lg border border-zinc-800 text-[10px] font-bold text-zinc-500 peer-checked:bg-white peer-checked:text-black transition-all flex items-center justify-center">{m}</div>
                  </label>
                ))}
              </div>
      ...
      {/* FOOTER NAV (位置をコンテンツ幅に同期) */}
      <nav className="fixed bottom-0 w-full max-w-md bg-[#121212] border-t border-white/5 flex justify-around p-3 pb-safe z-40 shadow-2xl">
