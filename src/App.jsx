      ...
      return (
        <div className="h-screen w-full bg-[#121212] text-zinc-200 flex flex-col items-center justify-center p-6 gap-4">
          ...
          <button
            ...
            className="px-6 py-3 bg-white text-black rounded-full font-bold text-sm uppercase"
          >
            再読み込み
          </button>
        </div>
...
function AppMain() {
  ...
  const getCategoryIcon = (n) => {
    const c = (config.categories || []).find(x => x.name === n);
    return c?.icon || '🏷';
  };
  ...
  const summary = useMemo(() => {
    ...
    const catBudgetSum = (config?.categories || []).reduce(
      (sum, c) => sum + (monthlyData?.catBudgets?.[c.name || c] || 0),
      0
    );
  ...
  // ✅ FIX #2: 当月のみ & bills/dueDates/confirmed をちゃんと参照してアラート算出
  ...
  // ✅ FIX #1: モーダルが落ちる原因（handleTxSubmit未定義）を実装
  const handleTxSubmit = async (e) => {
    e.preventDefault();
    if (!user) return;

    const amountNum = toNumber(inputAmount);
    const methodSafe = inputMethod || (config?.paymentMethods?.[0] || CASH);
    const catSafe = inputCategory || (getCategoryNames()[0] || '食費');

    if (!amountNum || !methodSafe || !catSafe || !inputDate) return;

    const payload = {
      date: new Date(`${inputDate}T00:00:00`).toISOString(),
      amount: amountNum,
      title: inputTitle || '',
      category: catSafe,
  ...
  const handleSettingsSave = async () => {
    ...
    if (type === 'category') {
      ...
      if (index === -1) newCats.push({ name: data.name, icon: data.icon });
      else newCats[index] = { name: data.name, icon: data.icon };
      ...
      if (data.budget) {
        await setDoc(
          doc(db, 'users', user.uid, 'months', month),
          { catBudgets: { ...(monthlyData.catBudgets || {}), [data.name]: toNumber(data.budget) } },
          { merge: true }
        );
      }
    } else if (type === 'fixed') {
      const newFixed = [...(monthlyData.fixedCosts || [])];
      if (index === -1) newFixed.push({ id: Date.now(), ...data, amount: toNumber(data.amount) });
      else newFixed[index] = { ...data, amount: toNumber(data.amount) };

      await setDoc(doc(db, 'users', user.uid, 'months', month), { fixedCosts: newFixed }, { merge: true });
    } else if (type === 'template') {
      const t = [...(config.templates || [])];
      if (index === -1) t.push({ ...data, amount: toNumber(data.amount) });
      else t[index] = { ...data, amount: toNumber(data.amount) };

      await setDoc(doc(db, 'users', user.uid, 'settings', 'config'), { ...config, templates: t });
    } else if (type === 'payment') {
      const p = [...(config.paymentMethods || [CASH])];
      if (index === -1) p.push(data.name);
      else p[index] = data.name;
  ...
  const handleDeleteItem = async () => {
    ...
    if (type === 'fixed') {
      ...
      await setDoc(
        doc(db, 'users', user.uid, 'settings', 'config'),
        { ...config, paymentMethods: (config.paymentMethods || []).filter((_, i) => i !== index) }
      );
    }
  ...
  const startEditing = (t) => {
    ...
    setInputCategory(t.category || (getCategoryNames()[0] || '食費'));
    setInputMethod(t.paymentMethod || (config?.paymentMethods?.[0] || CASH));
  ...
  const openModalWithDate = (dateStr) => {
    ...
    setInputMethod((config?.paymentMethods?.[0]) || CASH);
    setShowCalculator(false);
    setIsModalOpen(true);
  };
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        await setDoc(
          ...
          {
            budget: md.budget || 0,
            cashBudget: md.cashBudget || 0,
            fixedCosts: md.fixedCosts || [],
            catBudgets: md.catBudgets || {},
            cardBills: md.cardBills || {},
            cardDueDates: md.cardDueDates || {},
            // confirmedPayments はコピーしない（当月管理）
          },
          { merge: true }
        );
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      s.forEach(d => {
        ...
        csv += `${date},"${title}",${row.category || ''},${row.amount || 0},${row.paymentMethod || ''}\n`;
      });
  ...
  return (
    <div className="fixed inset-0 w-full bg-[#121212] text-zinc-200 font-sans font-bold flex flex-col justify-center overflow-hidden">
        ...
        <header className="flex-none h-16 border-b border-white/5 px-4 flex items-center justify-between bg-[#121212]/80 backdrop-blur-xl z-50">
          {activeTab === 'settings' && settingTab !== 'menu' ? (
            ...
            <>
              ...
              <button onClick={() => setMonth(getMonthString(new Date()))} className="text-zinc-500 active:text-white">
                <Calendar size={20}/>
              </button>
            </>
          )}
                        ...
                        return (
                          <SimpleCard key={n} className="p-3 space-y-2">
                            ...
                            <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                              <div className="h-full bg-zinc-500" style={{ width: `${Math.min(100, (s / b) * 100)}%` }} />
                            </div>
                          </SimpleCard>
                        );
                      })}
                        ...
                        {calendarDaysList.map((day, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {(a === 0 && !isT && new Date(month + '-' + String(day).padStart(2,'0')) <= new Date()) && (
                  ...
                  <div className="flex items-end justify-between gap-4">
                    <div>
                      ...
                      <div className={`flex items-center gap-1.5 mt-2 text-xs font-bold ${
                        summary.totalSpent <= summary.lastTotalSpent ? 'text-green-400' : 'text-red-400'
                      }`}>
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              onBlur={e => setDoc(doc(db, 'users', user.uid, 'months', month), {
                                salary: Number(String(e.target.value).replace(/,/g, ''))
                              }, { merge: true })}
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              onBlur={e => setDoc(doc(db, 'users', user.uid, 'months', month), {
                                budget: Number(String(e.target.value).replace(/,/g, ''))
                              }, { merge: true })}
                              className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white font-bold tabular-nums outline-none"
                            />
                          </div>
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              onBlur={e => setDoc(doc(db, 'users', user.uid, 'months', month), {
                                cashBudget: Number(String(e.target.value).replace(/,/g, ''))
                              }, { merge: true })}
                              className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white font-bold tabular-nums outline-none"
                            />
                          </div>
                          ...
                          {(config?.paymentMethods || []).filter(m => m !== CASH).map(m => (
                            <div key={m} className="flex gap-2 items-center">
                              ...
                              <input
                                ...
                                onBlur={e => {
                                  const newBills = { ...(monthlyData.cardBills || {}), [m]: Number(String(e.target.value).replace(/,/g, '')) };
                                  setDoc(doc(db, 'users', user.uid, 'months', month), { cardBills: newBills }, { merge: true });
                                }}
                              ...
                              <input
                                ...
                                onBlur={e => {
                                  const newDates = { ...(monthlyData.cardDueDates || {}), [m]: e.target.value };
                                  setDoc(doc(db, 'users', user.uid, 'months', month), { cardDueDates: newDates }, { merge: true });
                                }}
                    ...
                    {settingTab === 'fixed' && (
                      <SimpleCard className="p-5 animate-in slide-in-from-right-2">
                        ...
                        <button
                          ...
                          className="w-full h-11 bg-zinc-200 text-black rounded-lg mt-4 text-[10px] font-black uppercase shadow-lg active:scale-95"
                        >
                          固定費を追加
                        </button>
                      </SimpleCard>
                        ...
                        <div className="divide-y divide-white/5">
                          {(config?.categories || []).map((c, idx) => {
                            ...
                            return (
                              <div
                                key={idx}
                                onClick={() => setEditingItem({ type: 'category', data: { name: n, icon: getCategoryIcon(n), budget: b }, index: idx })}
                                className="flex justify-between items-center py-3 cursor-pointer"
                              >
                                <div className="flex items-center gap-3">
                                  <span className="text-xl w-8 text-center">{getCategoryIcon(n)}</span>
                                  <span className="text-xs font-bold text-white">{n}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] text-zinc-500 font-mono">¥{Number(b || 0).toLocaleString()}</span>
                                  <div className="flex gap-1">
                                    <button onClick={e => handleMoveCategory(idx, 'up', e)} className="p-1 text-zinc-600 hover:text-white"><ArrowUp size={14}/></button>
                                    <button onClick={e => handleMoveCategory(idx, 'down', e)} className="p-1 text-zinc-600 hover:text-white"><ArrowDown size={14}/></button>
                                  </div>
                                </div>
                              </div>
                            );
                            ...
                            >
                              <div className="flex flex-col">
                                ...
                                <span className="text-[9px] text-zinc-500 font-bold">
                                  ¥{Number(t.amount || 0).toLocaleString()} / {t.category}
                                </span>
                              </div>
                        ...
                        <button
                          onClick={() => setEditingItem({
                            ...
                            data: {
                              ...
                              method: config?.paymentMethods?.[0] || CASH
                            },
                            index: -1
                          })}
                          className="w-full h-11 bg-zinc-200 text-black rounded-lg mt-4 text-[10px] font-black uppercase shadow-lg active:scale-95"
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                          {(config?.paymentMethods || []).map((m, idx) => (
                            <div
                              ...
                              className="px-3 py-1.5 bg-white/5 rounded-lg border border-white/10 text-xs text-zinc-300 font-bold cursor-pointer active:scale-95 transition-all"
                            >
                              {m}
                            </div>
                          ))}
            ...
            {showCalculator ? (
              <div className="flex-1 p-5">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setShowCalculator(false)} className="text-zinc-500">
                    <X size={18}/>
                  </button>
                </div>
                <CalculatorPad
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-5 pb-8">
                  <form onSubmit={handleTxSubmit} className="space-y-6">
                    ...
                    {/* ✅ FIX #3: 日付とカテゴリの間隔を整える（grid + gap-3） */}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(config?.paymentMethods || [CASH]).map(m => (
      ...
      {editingItem && (
        ...
        >
          <div
            className="w-full max-h-[90vh] sm:h-auto sm:max-w-md bg-[#1E1E1E] sm:rounded-lg rounded-t-2xl border border-white/5 shadow-2xl flex flex-col"
            onClick={e => e.stopPropagation()}
          >
              ...
              {editingItem.type === 'fixed' && (
                <>
                  ...
                  <select
                    ...
                    className="w-full h-12 bg-black/20 border border-white/10 rounded-lg px-3 text-sm text-white outline-none font-bold"
                  >
                    {(config?.paymentMethods || [CASH]).map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                </>
              )}
