...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    // 起動直後でも拾えるように保存
    try {
      localStorage.setItem('ZAIMU_LAST_ERROR', JSON.stringify({
        message,
        stack,
  ...
  render() {
    if (this.state.hasError) {
      let saved = null;
      try {
        saved = JSON.parse(localStorage.getItem('ZAIMU_LAST_ERROR') || 'null');
      } catch {}

      const msg = this.state.message || saved?.message || '不明なエラー';
      const st = this.state.stack || saved?.stack || '';
      ...
      return (
        <div className="h-screen w-full bg-[#121212] text-zinc-200 flex flex-col items-center justify-center p-6 gap-4">
          ...
          <p className="text-xs text-zinc-500 text-center">
            下のエラー内容をコピーして貼ってくれれば、原因をピンポイントで直せる！
          </p>

          <div className="w-full max-w-md bg-black/40 border border-white/10 rounded-lg p-3 text-left">
            <div className="text-[11px] text-zinc-200 break-words">{msg}</div>
            {st ? (
              <pre className="mt-2 text-[10px] text-zinc-500 whitespace-pre-wrap break-words">
                {st}
              </pre>
            ) : null}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-6 py-3 bg-white/10 text-white rounded-full font-bold text-sm uppercase active:scale-95 transition-transform border border-white/10"
            >
              再読み込み
            </button>
          </div>
                    ...
                    <SimpleCard>
                      {finalFilteredTx.length === 0 ? (
                        <div className="py-20 flex flex-col items-center gap-3 text-zinc-600">
                          ...
                          <p className="text-xs uppercase font-black">履歴がありません</p>
                        </div>
                      ) : (
                ...
                {settingTab === 'menu' ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <SimpleCard className="p-5 text-xs text-zinc-500">
                      ※ このファイルは「起動クラッシュの原因表示」を優先して ErrorBoundary を強化しています。<br/>
                      次のターンで、表示されたエラー文を貼ってくれれば、原因を潰した“本命の全量”で返します。
                    </SimpleCard>
                  </div>
                )}
              </div>
