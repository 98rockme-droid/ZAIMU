      ...
      <main className="w-full max-w-md p-4 space-y-4 box-border">
          ...
          <div className="space-y-3 animate-in fade-in">
            ...
            {transactions.filter(t => (filter.category === 'ALL' || t.category === filter.category) && (filter.method === 'ALL' || t.paymentMethod === filter.method)).map(t => (
              <div key={t.id} className="flex justify-between items-center bg-[#1E1E1E] p-4 rounded-lg border border-white/5 font-bold">
                ...
                <div className="flex items-center gap-3 pl-2 font-bold">
        ...
        {activeTab === 'settings' && (
          <div className="space-y-4 animate-in fade-in font-bold">
                  ...
                  <div className="space-y-4 font-bold">
                    {config.paymentMethods.filter(m => m !== '現金').map(card => (
                      <div key={card} className="flex flex-col gap-1 font-bold">
                        <label className="text-[10px] text-zinc-500 text-left font-bold">{card}</label>
                        <input 
                          type="number" 
                          ...
                          className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-left text-white outline-none font-mono box-border font-bold" 
                          placeholder="0"
                        />
                      </div>
                    ))}
              ...
              <SimpleCard className="p-6 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col gap-2 pt-6">
                    <input id="fx-n" placeholder="費目 (例: 家賃)" className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none text-left font-bold" />
                    <input id="fx-a" type="number" placeholder="金額" className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none text-left font-mono font-bold" />
                    <button onClick={() => {
            ...
            {settingTab === 'category' && (
              <SimpleCard className="p-6 space-y-6 font-bold">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-2 pt-2">
                    <input id="new-cat" placeholder="新カテゴリ" className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none box-border font-bold" />
                    <button onClick={() => {
                      ...
                      i.value='';
                    }} className="w-full h-11 bg-zinc-200 text-black rounded-lg font-bold text-xs uppercase tracking-widest">追加</button>
                  </div>
                </div>
              </SimpleCard>
            )}
                ...
                <div className="space-y-4 font-bold">
                  <div className="flex flex-wrap gap-2 justify-start font-bold">
                    {config.paymentMethods.map(p => (
                      <div key={p} className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10 font-bold">
                  ...
                  <div className="flex flex-col gap-2 pt-2">
                    <input id="new-pay" placeholder="カード名等" className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white outline-none box-border font-bold" />
                    <button onClick={() => {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <SimpleCard className="relative w-full max-w-md p-6 space-y-6 box-border font-bold">
            <div className="flex justify-between items-center font-bold font-bold"><h2 className="text-xs font-bold uppercase text-white tracking-widest">支出入力</h2><button onClick={() => setIsModalOpen(false)}><X size={20}/></button></div>
            <form onSubmit={handleTxSubmit} className="space-y-5">
              <input name="amount" type="number" defaultValue={editingTx?.amount || ''} className="w-full h-16 bg-black/20 border border-white/10 rounded-lg text-3xl font-bold text-left px-4 text-white outline-none box-border font-bold" placeholder="0" autoFocus required />
              <input name="title" type="text" defaultValue={editingTx?.title || ''} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-left text-white outline-none box-border font-bold" placeholder="タイトル (例: ランチ)" />
              <div className="flex flex-row gap-4 w-full">
                <div className="flex-1 flex flex-col gap-1 overflow-hidden">
                  <label className="text-[10px] text-zinc-500 font-bold uppercase pl-1 font-bold">日付</label>
                  <input name="date" type="date" defaultValue={editingTx ? editingTx.date.split('T')[0] : getTodayString()} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg text-xs text-left px-2 text-white outline-none box-border appearance-none font-bold" />
                </div>
                <div className="flex-1 flex flex-col gap-1 overflow-hidden font-bold">
                  <label className="text-[10px] text-zinc-500 font-bold uppercase pl-1 font-bold">カテゴリ</label>
                  <select name="category" defaultValue={editingTx?.category || config.categories[0]} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg text-xs text-left px-2 text-white outline-none appearance-none box-border font-bold font-bold">
                    {config.categories.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
              </div>
              <div className="flex flex-wrap gap-2 justify-start font-bold">
                {config.paymentMethods.map(m => (
                  <label key={m} className="cursor-pointer font-bold">
                    ...
                    <div className="px-4 h-10 text-center rounded-lg border border-zinc-800 text-[10px] font-bold text-zinc-500 peer-checked:bg-white peer-checked:text-black transition-all flex items-center justify-center min-w-[60px] font-bold font-bold">{m}</div>
                  </label>
                ))}
              </div>
