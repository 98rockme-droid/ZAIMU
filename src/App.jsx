...
/* ✅ FIX(①): config.categories が「文字列」混在でも落ちないように正規化 */
const normalizeConfig = (data) => {
  const rawCats = data?.categories || [{ name: '食費', icon: '🍔' }];
  ...
  const rawMethods = data?.paymentMethods || [CASH];
  const paymentMethods = rawMethods.filter(Boolean);
...
function AppMain() {
  ...
  /* ✅ FIX(②): 資金計画のカード請求/引落日の draft state を追加 */
  const [draftCardBills, setDraftCardBills] = useState({});
  const [draftCardDueDates, setDraftCardDueDates] = useState({});
  ...
  /* ✅ FIX(②): budget タブを開いたとき draft を monthlyData で同期 */
  useEffect(() => {
    if (!user) return;
    if (activeTab !== 'settings') return;
    if (settingTab !== 'budget') return;

    setDraftCardBills(monthlyData.cardBills || {});
    setDraftCardDueDates(monthlyData.cardDueDates || {});
  }, [user, activeTab, settingTab, monthlyData.cardBills, monthlyData.cardDueDates]);
  ...
  /* ✅ FIX(①): ここも安全化（分析タブでの map が落ちるのを防止） */
  ...
  const handleTxSubmit = async (e) => {
    ...
    try {
      ...
      const dateISO = new Date(`${inputDate}T00:00:00`).toISOString();

      const payload = {
        amount: amountNum,
        title: inputTitle || '',
        category: inputCategory || (getCategoryNames()[0] || '食費'),
        paymentMethod: inputMethod || (config?.paymentMethods?.[0] || CASH),
        date: dateISO,
        updatedAt: serverTimestamp()
      };

      if (editingTx?.id) {
        await setDoc(doc(db, 'users', user.uid, 'transactions', editingTx.id), payload, { merge: true });
      } else {
                  ...
                  {logView === 'list' ? (
                    <SimpleCard>
                      {finalFilteredTx.length === 0
                        ? <div className="py-20 flex flex-col items-center gap-3 text-zinc-600"><Sparkles size={48} className="opacity-20" /><p className="text-xs uppercase font-black">No Spending! 🎉</p></div>
                        : <div className="divide-y divide-white/5">{finalFilteredTx.map(t => (
              ...
              <div className="animate-in fade-in">
                {settingTab === 'menu' ? (
                  <div className="space-y-6 pb-10">
                    ...
                    <div className="bg-[#1E1E1E] rounded-xl border border-white/5 overflow-hidden"><div className="divide-y divide-white/5"> {SETTING_MENU_ITEMS.map(item => (
                      <button key={item.id} onClick={() => setSettingTab(item.id)} className="w-full flex items-center justify-between p-4 active:bg-white/5 text-zinc-300 transition-colors">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-zinc-500 font-mono">{(item.id === 'fixed' ? `¥${summary.fixedTotal.toLocaleString()}` : item.id === 'category' ? `¥${summary.catBudgetSum.toLocaleString()}` : '')}<ChevronRight size={16} className="text-zinc-800" /></div>
                      </button>
                    ))} </div></div>
                      ...
                      <SimpleCard className="p-5 space-y-4 animate-in slide-in-from-right-2">
                        <div className="space-y-3">
                          <div className="flex flex-col gap-1">
                            ...
                            <input type="text" inputMode="decimal" defaultValue={monthlyData.salary?.toLocaleString()} onBlur={e => setDoc(doc(db, 'users', user.uid, 'months', month), { salary: Number(e.target.value.replace(/,/g, '')) }, { merge: true })} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white font-bold tabular-nums outline-none" />
                          </div>
                          <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-zinc-600 font-bold uppercase pl-1">生活費予算（総枠）</label>
                            <input type="text" inputMode="decimal" defaultValue={monthlyData.budget?.toLocaleString()} onBlur={e => setDoc(doc(db, 'users', user.uid, 'months', month), { budget: Number(e.target.value.replace(/,/g, '')) }, { merge: true })} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white font-bold tabular-nums outline-none" />
                          </div>
                          <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-zinc-600 font-bold uppercase pl-1">現金予算（口座用）</label>
                            <input type="text" inputMode="decimal" defaultValue={monthlyData.cashBudget?.toLocaleString()} onBlur={e => setDoc(doc(db, 'users', user.uid, 'months', month), { cashBudget: Number(e.target.value.replace(/,/g, '')) }, { merge: true })} className="w-full h-11 bg-black/20 border border-white/10 rounded-lg px-4 text-sm text-white font-bold tabular-nums outline-none" />
                          </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-white/5 space-y-3">
                          ...
                          {/* ✅ FIX(②): defaultValue/onBlur 依存をやめて draft(value) で確実に保存 */}
                          {(config?.paymentMethods || []).filter(m => m !== CASH).map(m => {
                            const billVal = draftCardBills?.[m] ?? 0;
                            const dueVal = draftCardDueDates?.[m] ?? '';
                            ...
                            return (
                              <div key={m} className="flex gap-2 items-center">
                                ...
                                <input
                                  type="text"
                                  inputMode="decimal"
                                  value={Number(billVal || 0).toLocaleString()}
                                  onChange={e => {
                                    const raw = e.target.value.replace(/,/g, '');
                                    if (raw === '' || !isNaN(raw)) {
                                      setDraftCardBills(prev => ({ ...prev, [m]: Number(raw || 0) }));
                                    }
                                  }}
                                  onBlur={async () => {
                                    try {
                                      await setDoc(
                                        ...
                                        { cardBills: { ...(draftCardBills || {}), [m]: Number(draftCardBills?.[m] || 0) } },
                                        { merge: true }
                                      );
                                      showToastMsg('保存しました');
                                    } catch (e) {
                                ...
                                <input
                                  ...
                                  onChange={e => {
                                    ...
                                    setDraftCardDueDates(prev => ({ ...prev, [m]: v === '' ? '' : Number(v) }));
                                  }}
                                  onBlur={async () => {
                                    try {
                                      const num = Number(draftCardDueDates?.[m]);
                                      const safe = Number.isFinite(num) ? num : '';

                                      await setDoc(
                                        doc(db, 'users', user.uid, 'months', month),
                                        { cardDueDates: { ...(draftCardDueDates || {}), [m]: safe } },
                                        { merge: true }
                                      );
                                      showToastMsg('保存しました');
                                    } catch (e) {
