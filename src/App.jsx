...
/* --- CONSTANTS & UTILS --- */
const CASH = '現金'; // FIX: Magic string constant
...
const toNumber = (val) => { // FIX: Safe number conversion
  if (!val) return 0;
  const num = Number(String(val).replace(/,/g, ''));
  return isNaN(num) ? 0 : num;
};

// FIX: Data Normalizers to prevent undefined errors
...
/* --- COMPONENTS --- */
// FIX: ErrorBoundary to prevent white screen of death
    ...
    if (this.state.hasError) {
      return (
        <div className="h-screen w-full bg-[#121212] text-zinc-200 flex flex-col items-center justify-center p-6 gap-4">
          ...
          <p className="text-xs text-zinc-500">アプリを再読み込みしてください。</p>
...
function AppContent() {
  ...
  // REFACTOR: Split useEffects for better maintenance and performance
  useEffect(() => {
    if (!user) return;
  ...
  // REFACTOR: Logic split for summary
  ...
  const handleSettingsSave = async () => {
    ...
    // ... logic same as previous but robust with normalized config
    if (type === 'category') {
        const newCats = [...config.categories];
