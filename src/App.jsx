...
const SettingsRow = ({ left, right, onClick }) => (
    ...
    >
      ...
      {right ? <div className="text-[11px] text-zinc-500 shrink-0">{right}</div> : <div />}
    </button>
);
...
function AppMain() {
  ...
  const openModalNew = () => {
    setEditingTx(null); setInputDate(getTodayString()); setInputAmount(''); setInputTitle(''); setInputCategory(getCategoryNames()[0] || '食費'); setInputMethod(config.paymentMethods[0] || CASH); setShowCalculator(false); setIsModalOpen(true);
  };
    ...
    <div className="fixed inset-0 w-full bg-[#121212] text-zinc-200 font-sans font-bold flex flex-col justify-center overflow-hidden">
      ...
      <div className="w-full max-w-md h-full flex flex-col relative bg-[#121212] shadow-2xl mx-auto">
                   ...
                   <div className="space-y-3">
                     ...
                     <div className="flex gap-2">
                       <div className="relative flex-1">
                           <select onChange={e => setFilter({...filter, category: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-2 h-9 text-[10px] text-zinc-300 outline-none appearance-none"><option value="ALL">全てのカテゴリ</option>{getCategoryNames().map(c => <option key={c} value={c}>{c}</option>)}</select>
                           <ChevronDown size={14} className="absolute right-2 top-2.5 text-zinc-500 pointer-events-none"/>
                       </div>
                       <div className="relative flex-1">
                           <select onChange={e => setFilter({...filter, method: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-2 h-9 text-[10px] text-zinc-300 outline-none appearance-none"><option value="ALL">全ての支払方法</option>{(config?.paymentMethods||[]).map(m => <option key={m} value={m}>{m}</option>)}</select>
                           <ChevronDown size={14} className="absolute right-2 top-2.5 text-zinc-500 pointer-events-none"/>
                       </div>
                     </div>
                   </div>
                        ...
                        <div className="space-y-4 animate-in slide-in-from-right-2">
                             <SimpleCard className="overflow-hidden">
                                <div className="divide-y divide-white/5">
                                    <SettingsRow onClick={() => openEdit('budget', { salary: monthlyData.salary, budget: monthlyData.budget, cashBudget: monthlyData.cashBudget }, 0)} left={<span className="text-sm text-zinc-200">資金計画</span>} right={null} />
                                </div>
                             </SimpleCard>
                             <SimpleCard className="overflow-hidden">
                                <div className="divide-y divide-white/5">
                                    {(config?.paymentMethods||[]).filter(m=>m!==CASH).map(m=>(<SettingsRow key={m} onClick={() => openEdit('bill', { name: m, bill: monthlyData.cardBills?.[m] ?? 0, due: monthlyData.cardDueDates?.[m] ?? '' }, 0)} left={<span className="text-sm text-zinc-200">{m}</span>} right={<div className="flex items-center gap-2"><span className="text-xs text-zinc-400 tabular-nums">¥{Number(monthlyData.cardBills?.[m] || 0).toLocaleString()}</span><span className="text-xs text-zinc-500 tabular-nums">{String(monthlyData.cardDueDates?.[m] || '-') }日</span></div>} />))}
                                </div>
                             </SimpleCard>
                        </div>
                    )}
                    {settingTab === 'fixed' && <div className="space-y-3 animate-in slide-in-from-right-2"><button type="button" onClick={()=>openEdit('fixed',{name:'',amount:'',method:CASH},-1)} className="w-full h-12 bg-white text-black rounded-lg text-xs tracking-widest active:scale-95 flex items-center justify-center gap-2"><Plus size={16}/> 追加</button><SimpleCard className="overflow-hidden"><div className="divide-y divide-white/5">{(monthlyData.fixedCosts || []).map((f,idx)=>(<SettingsRow key={f.id||idx} onClick={() => openEdit('fixed', f, idx)} left={<div className="flex items-center gap-2 min-w-0 text-left"><span className="text-[9px] px-2 py-1 rounded bg-white/5 text-zinc-400 shrink-0">{f.method||'未設定'}</span><span className="text-sm text-zinc-200 truncate text-left">{f.name}</span></div>} right={<span className="text-sm text-white tabular-nums">¥{Number(f.amount||0).toLocaleString()}</span>} />))}</div></SimpleCard></div>}
                    {settingTab === 'category' && <div className="space-y-3 animate-in slide-in-from-right-2"><button type="button" onClick={()=>openEdit('category',{name:'',icon:'🏷',budget:''},-1)} className="w-full h-12 bg-white text-black rounded-lg text-xs tracking-widest active:scale-95 flex items-center justify-center gap-2"><Plus size={16}/> 追加</button><SimpleCard className="overflow-hidden"><div className="divide-y divide-white/5">{(config?.categories || []).map((c,idx)=>{const n=c.name; const b=monthlyData.catBudgets?.[n]||0; return(<SettingsRow key={n} onClick={() => openEdit('category', { ...c, budget: b }, idx)} left={<div className="flex items-center gap-3"><span className="text-xl w-8 text-center">{c.icon||'🏷'}</span><span className="text-sm text-zinc-200 text-left">{n}</span></div>} right={<span className="text-xs text-zinc-400 tabular-nums">¥{Number(b).toLocaleString()}</span>} />)})}</div></SimpleCard></div>}
                    {settingTab === 'template' && <div className="space-y-3 animate-in slide-in-from-right-2"><button type="button" onClick={()=>openEdit('template',{title:'',amount:'',category:getCategoryNames()[0]||'食費',method:config?.paymentMethods?.[0]||'現金'},-1)} className="w-full h-12 bg-white text-black rounded-lg text-xs tracking-widest active:scale-95 flex items-center justify-center gap-2"><Plus size={16}/> 追加</button><SimpleCard className="overflow-hidden"><div className="divide-y divide-white/5">{(config?.templates || []).map((t,idx)=>(<SettingsRow key={idx} onClick={() => openEdit('template', t, idx)} left={<div className="flex flex-col items-start text-left min-w-0"><span className="text-sm text-zinc-200 truncate text-left">{t.title}</span><span className="text-[10px] text-zinc-500">{t.category} / {t.method}</span></div>} right={<span className="text-xs text-zinc-400 tabular-nums">¥{Number(t.amount||0).toLocaleString()}</span>} />))}</div></SimpleCard></div>}
                    {settingTab === 'payment' && <div className="space-y-3 animate-in slide-in-from-right-2"><button type="button" onClick={()=>openEdit('payment',{name:''},-1)} className="w-full h-12 bg-white text-black rounded-lg text-xs tracking-widest active:scale-95 flex items-center justify-center gap-2"><Plus size={16}/> 追加</button><SimpleCard className="overflow-hidden"><div className="divide-y divide-white/5">{(config?.paymentMethods || []).map((m, idx) => (<SettingsRow key={m} onClick={() => openEdit('payment', { name: m }, idx)} left={<span className="text-sm text-zinc-200 text-left">{m}</span>} right={null} />))}</div></SimpleCard></div>}
                  </div>
                )}
              </div>
            )}
        ...
        <footer className="flex-none h-24 border-t border-white/5 flex justify-between items-center px-6 pb-6 bg-[#121212]/80 backdrop-blur-xl z-50">
          ...
          <button onClick={openModalNew} className="flex items-center justify-center w-14 h-14 bg-white text-black rounded-full shadow-[0_0_20px_rgba(255,255,255,0.2)] active:scale-90 ml-2 transition-transform"><Plus size={28}/></button>
        </footer>
      </div>
