...
export default function App() {
  ...
  const openModalWithDate = (dateStr) => {
    ...
    setShowCalculator(false); // 電卓ではなく
    setIsModalOpen(true); // 登録モーダルを開く
  };
          ...
          {(activeTab === 'settings' && settingTab !== 'menu') ? (
            ...
            <>
                ...
                <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center">
                    ...
                    {/* Header Totals */}
                    {settingTab === 'fixed' && <span className="text-[10px] text-zinc-400 tabular-nums">計 ¥{summary.fixedTotal.toLocaleString()}</span>}
                    {settingTab === 'category' && <span className="text-[10px] text-zinc-400 tabular-nums">計 ¥{summary.catBudgetSum.toLocaleString()}</span>}
                </div>
                    ...
                    <SimpleCard className="p-4">
                      ...
                      <div className="grid grid-cols-7 gap-1">
                        {calendarDays.map((day, i) => {
                          ...
                          const isNMD = amt === 0 && !isFuture && !isToday;
                        ...
                        <div key={catName} className="space-y-2">
                          <div className="flex justify-between items-center font-bold">
                            ...
                            <div className="flex gap-3 text-[10px] tabular-nums">
                                <span className="text-zinc-600">先月 ¥{last.toLocaleString()}</span>
                          ...
                          <div className="space-y-1">
                              ...
                              {/* Last Month Ghost Bar */}
                              <div className="h-1 bg-white/5 rounded-full overflow-hidden opacity-30"><div className="h-full bg-zinc-400 rounded-full transition-all duration-1000" style={{ width: `${(last / max) * 100}%` }} /></div>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    {settingTab === 'menu' && (
                      <div className="space-y-6 pb-10">
                        ...
                        <div className="flex flex-col gap-1 px-2">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    {user.photoURL ? (
                                        <img src={user.photoURL} referrerPolicy="no-referrer" alt="icon" className="w-8 h-8 rounded-full object-cover" />
                                    ) : (
                        ...
                        <div className="bg-[#1E1E1E] rounded-xl border border-white/5 overflow-hidden">
                            <div className="divide-y divide-white/5">
                                {[{ id: 'budget', label: '資金計画・引き落とし日', icon: <Landmark size={18}/> }, { id: 'fixed', label: '固定費管理', icon: <CreditCard size={18}/>, value: `¥${summary.fixedTotal.toLocaleString()}` }, { id: 'category', label: 'カテゴリ・予算管理', icon: <Tags size={18}/>, value: `¥${summary.catBudgetSum.toLocaleString()}` }, { id: 'template', label: 'テンプレート編集', icon: <Zap size={18}/> }, { id: 'payment', label: '支払方法・カード編集', icon: <Wallet size={18}/> }].map(item => (
                                  <button key={item.id} onClick={() => { setSettingTab(item.id); }} className="w-full flex items-center justify-between p-4 active:bg-white/5 transition-colors text-zinc-300">
