...
const callGemini = async (prompt) => {
  ...
  try {
    ...
    return text ? JSON.parse(text) : { message: "順調な管理ですね。" };
  } catch (error) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // デバイス間で共有されやすいよう、共通のコレクションパスを指定
  ...
  if (loading) return <div className="h-screen bg-[#121212] flex items-center justify-center text-zinc-600 text-xs tracking-widest font-bold">システムを同期中...</div>;

  return (
    <div className="min-h-screen bg-[#121212] text-zinc-200 font-sans pb-24">
      {/* HEADER */}
      <header className="sticky top-0 z-30 bg-[#121212]/95 backdrop-blur-md border-b border-white/5 p-4 flex flex-col gap-3">
        <div className="flex justify-between items-center">
          <h1 className="text-xl font-black tracking-tighter text-white">ZAIMU</h1>
          <div className="flex items-center gap-2">
            <button 
              onClick={goCurrentMonth}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-full border border-white/10 text-[10px] font-black text-zinc-400 hover:text-white transition-all uppercase tracking-widest"
            >
              <CalendarDays size={12} /> 今月
            </button>
      ...
      <main className="max-w-md mx-auto p-4 space-y-5">
        ...
        {activeTab === 'log' && (
          <div className="space-y-2 animate-in fade-in">
            ...
            {transactions.length === 0 && <p className="text-center text-zinc-600 py-20 text-xs font-bold uppercase tracking-widest font-mono">データなし</p>}
          </div>
        )}
          ...
          <SimpleCard className="p-6 animate-in slide-in-from-bottom-4">
            ...
            <form onSubmit={handleTxSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {config.categories.map(c => (
                    <label key={c} className="cursor-pointer">
                      ...
                      <div className="py-2 text-center rounded-lg border border-zinc-800 text-[11px] font-bold text-zinc-500 peer-checked:bg-zinc-200 peer-checked:text-black peer-checked:border-white transition-all">{c}</div>
                    </label>
                  ))}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {config.paymentMethods.map(m => (
                    <label key={m} className="cursor-pointer">
                      ...
                      <div className="py-2 text-center rounded-lg border border-zinc-800 text-[11px] font-bold text-zinc-500 peer-checked:bg-zinc-700 peer-checked:text-white transition-all">{m}</div>
                    </label>
                  ))}
                </div>
              </div>
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" className="flex-2 bg-zinc-200 text-black font-black rounded-xl py-4 px-8 text-xs shadow-lg uppercase tracking-widest">保存</button>
              </div>
            </form>
          </SimpleCard>
        )}
          ...
          <div className="space-y-4 animate-in fade-in">
            <SimpleCard className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] text-zinc-500 mb-2 block uppercase font-bold">今月の軍資金</label>
                  <input type="number" value={monthlyData.budget} onChange={e => setDoc(doc(db, 'users', user.uid, 'months', month), { budget: Number(e.target.value) }, { merge: true })} className="w-full bg-black/20 border border-white/10 rounded-lg p-4 text-white font-mono text-sm outline-none" />
                </div>
                <div>
                  <label className="text-[10px] text-zinc-500 mb-2 block uppercase font-bold">前月のカード請求額</label>
                  <input type="number" value={monthlyData.cardBill} onChange={e => setDoc(doc(db, 'users', user.uid, 'months', month), { cardBill: Number(e.target.value) }, { merge: true })} className="w-full bg-black/20 border border-white/10 rounded-lg p-4 text-white font-mono text-sm outline-none" />
                </div>
              </div>
            </SimpleCard>

            <SimpleCard className="p-6">
              <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.3em] mb-4 text-white">固定費設定</h3>
              <div className="space-y-2">
                ...
                <div className="flex gap-2 pt-4">
                  <input id="fx-n" placeholder="家賃等" className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-xs outline-none" />
                  ...
                  <button onClick={() => {
                    ...
                    n.value=''; a.value='';
                  }} className="bg-zinc-200 text-black px-4 rounded-lg font-bold text-xs">追加</button>
                </div>
              </div>
            </SimpleCard>
          </div>
